...
export function useProjectSocket(projectId: string | undefined) {
  ...
  useEffect(() => {
    ...
    socket.on('code_synced', (payload: CodeSyncedPayload) => {
      ...
      // Don't apply our own updates (we already have them locally)
    ...
    socket.on('cursor_move', (payload: CursorMovePayload) => {
      if (payload.projectId !== projectId) return
      if (payload.userId === user?.id) return
      const userEmail = useEditorStore.getState().activeUsers.find(u => u.id === payload.userId)?.email ?? payload.userId
